...
docker run --rm -v "$NET:/workspace" -w /workspace hyperledger/fabric-tools:2.5.12 cryptogen generate --config=crypto-config.yaml --output=organizations
docker run --rm -v "$NET:/workspace" -w /workspace -e FABRIC_CFG_PATH=/workspace hyperledger/fabric-tools:2.5.12 configtxgen -profile ReliefApplicationChannel -channelID reliefchannel -outputBlock channel-artifacts/reliefchannel.block
