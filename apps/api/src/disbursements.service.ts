...
export class DisbursementsService {
  ...
  async initiate(raw: any, user: SessionUser) {
    ...
    const reservation = await this.db.query<any>(`INSERT INTO disbursement_requests(idempotency_key,request_hash,status) VALUES($1,$2,'PROCESSING') ON CONFLICT(idempotency_key) DO NOTHING RETURNING idempotency_key`, [input.idempotencyKey, requestHash]);
