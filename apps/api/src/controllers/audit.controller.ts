...
export class AuditController {
  constructor(private db: DatabaseService, private ledgerEvents: LedgerRepository, private rateLimit: RateLimitService) {}

  @Get('events')
  async events(@Query('type') type?: string, @Query('before') before?: string, @Query('limit') limit = '100', @Req() req?: any) {
    await this.rateLimit.check('audit-events', req?.ip ?? 'unknown', Number(process.env.AUDIT_FILTER_RATE_LIMIT ?? 60), 60_000);
    const page = await this.ledgerEvents.listEvents({ type, beforeSequence: before ? Number(before) : undefined, limit: Number(limit) || 100 });
    return before ? page : page.items;
  }
  ...
  async reconciliation() {
    const q = await this.db.query<any>(`SELECT fs.id,fs.name,fs.source_type,fs.amount_paise,fs.allocated_paise,
      ...
      FROM fund_sources fs LEFT JOIN allocations a ON a.source_id=fs.id GROUP BY fs.id ORDER BY fs.created_at`);
  ...
  async csv(@Req() req: any) {
    ...
    return ['public_reference,district_code,scheme,source_type,amount_paise,status,created_at,updated_at,transaction_id', ...q.rows.map((row) => Object.values(row).map(escape).join(','))].join('\n');
  }
}
