...
export class AuditController {
	...
	@Get('events') async events(@Query('type') type?: string, @Query('limit') limit = '100') { const params: unknown[] = []; let where = ''; if (type) { params.push(type); where = 'WHERE entity_type=$1'; } params.push(Math.min(Number(limit) || 100, 500)); const q = await this.db.query(`SELECT * FROM ledger_events ${where} ORDER BY sequence DESC LIMIT $${params.length}`, params); return q.rows; }
