...
export interface PayoutProvider { submit(input: { disbursementId: string; amountPaise: number; requestedOutcome: ProviderStatus }): Promise<ProviderResult>; reconcile(providerReference: string): Promise<ProviderResult> }